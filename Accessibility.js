 ...
 * @text Tracked Quest Text
 * @desc For use with the Galv_QuestLog plugin.
 * @type string
 * @default Tracked
 *
 * @param completedQuestText
 * @text Completed Quest Text
 * @desc For use with the Galv_QuestLog plugin.
 * @type string
 * @default Completed
 *
 * @param failedQuestText
 * @text Failed Quest Text
 * @desc For use with the Galv_QuestLog plugin.
 * @type string
 * @default Failed
 *
 ...
 * @text Required Actor Text
 * @desc For use with the YEP_PartySystem plugin.
 * @type string
 * @default Required
 *
 * @param announceLeaderHp
 * @text Announce Leader HP
 * @desc Only applies outside battles. In battle each member's HP is always announced.
 * @type boolean
 * @default true
 *
 * @param announceGold
 ...
 * @text Announce Enemy HP
 * @desc No setting is required when using any of the FTKR_DisplayEnemyParameters, MOG_HPGauge or YEP_X_VisualHpGauge plugins.
 * @type boolean
 * @default false
 *
 * @param announceEnemyMp
 * @text Announce Enemy MP
 * @desc No setting is required when using the FTKR_DisplayEnemyParameters plugin.
 * @type boolean
 * @default false
 *
 * @param announceEnemyTp
 * @text Announce Enemy TP
 * @desc No setting is required when using the FTKR_DisplayEnemyParameters plugin.
 * @type boolean
 * @default false
 *
 * @param stripItemCategoryEscapes
 * @text Strip Item Category Escapes
 * @desc E.g. \I.
 * @type boolean
 * @default false
 *
 * @param stripSkillTypeEscapes
 * @text Strip Skill Type Escapes
 * @desc E.g. \I.
 * @type boolean
 * @default false
 *
 * @param stripEquipCommandEscapes
 * @text Strip Equip Command Escapes
 * @desc E.g. \I.
 * @type boolean
 * @default false
 *
 * @param stripShopCommandEscapes
 * @text Strip Shop Command Escapes
 * @desc E.g. \I.
 * @type boolean
 * @default false
 *
 * @param stripPartyCommandEscapes
 * @text Strip Party Command Escapes
 * @desc E.g. \I.
 * @type boolean
 * @default false
 *
 * @param stripActorCommandEscapes
 * @text Strip Actor Command Escapes
 * @desc E.g. \I.
 * @type boolean
 * @default false
 *
 * @param stripBattleLogTextAlignTags
 * @text Strip Battle Log Text Alignment Tags
 * @desc E.g. <LEFT>, <CENTER>, <RIGHT>.
 * @type boolean
 * @default false
 *
 * @param eventToObjectId
 * @text Event To Object ID
 * @desc In the object list, events with empty object IDs are hidden, and adjacent events with the same object ID are merged into one entry. Has higher priority than the “AccessibleName” tag.
 * @type struct<eventToObjectId>[]
 * @default []
 *
 * @param objectIdToName
 * @text Object ID To Name
 ...
 * @text Disable Object List Map IDs
 * @type number[]
 * @default []
 *
 * @param playerCoordsFormat
 * @text Player Coordinates Format
 * @desc %1: Map ID, %2: X Coordinate, %3: Y Coordinate.
 * @type string
 * @default Map #%1 Coords %2, %3
 *
 * @param objectListItemFormat
 * @text Object List Item Format
 * @desc %1: Name, %2: X Coordinate, %3: Y Coordinate.
 * @type string
 * @default %1: %2, %3
 *
 ...
 * @type string
 * @default Accessible Touch Mode
 *
 * @param debugMode
 * @text Debug Mode
 * @desc Display output text at the top left of the screen, and output warnings for certain unimplemented features.
 * @type boolean
 * @default false
 *
 * @help
 ...
 * @param event
 * @text Event
 * @desc Formats (in order of decreasing priority): “Map ID:Event ID”, “Map ID:Event Name” or “Event Name”.
 * @type string
 *
 * @param objectId
 * @text Object ID
 ...
 * @param eventToObjectId
 * @text 事件的对象 ID
 * @desc 对象列表中，对象 ID 为空的事件会被隐藏，对象 ID 相同的相邻事件会被合并为一项。优先级高于“AccessibleName”标签。
 * @type struct<eventToObjectId>[]
 * @default []
 *
 * @param objectIdToName
...
self.Accessibility = (() => {
  ...
  const hasDrillGaugeFloatingPermanentText = typeof DrillUp !== "undefined" &&
    !!DrillUp.g_GFPT_PluginTip_curName;
  ...
  function getObjectId(data, mapId, event) {
    ...
    if (typeof DrillUp !== "undefined" && DrillUp.g_ET_PluginTip_curName) {
  ...
  const describeObject = ({ name, description, count, params }) => {
    let text = name;
    if (count !== undefined) {
      text += ` × ${count}`;
    }
  ...
  function clearGlobalState() {
    ...
    for (const child of customStatusNode.childNodes) {
    ...
    postfix() {
      ...
      const name = isMV
        ? typeof Yanfly !== "undefined" && Yanfly.nameWindow &&
          ...
          : this._drill_DNB_nameWindow && this._drill_DNB_nameWindow.active
          ? stripEscapes(this._drill_DNB_nameWindow._drill_showingText)
  ...
  if (hasCGMVEncyclopedia) {
    const getPercentCompletion = (symbol) => {
      ...
      const discovered = $cgmv.getEncyclopediaDiscovered(symbol);
      return (discovered / total * 100).toFixed(CGMV.Encyclopedia.DecimalSpots);
    };

    Patcher.patch(CGMV_Window_EncyclopediaTotals.prototype, "refresh", {
      postfix() {
        const params = [
          `${this._name}${getPercentCompletion(this._symbol)}%`,
          `${CGMV.Encyclopedia.TotalText}${getPercentCompletion("total")}%`,
        ];
        setTextIfChanged(notesNode, params.join(", "));
      },
    });

    const describeEntry = (symbol, data) => {
      if (!data._discovered) {
        return CGMV.Encyclopedia.UnknownEntryDisplay;
      }
      switch (symbol) {
        case "bestiary":
          return describeUnimplementedCase("bestiary");
        case "items":
          return describeUnimplementedCase("items");
        case "armors":
          return describeUnimplementedCase("armors");
        case "weapons":
          return describeUnimplementedCase("weapons");
        case "skills":
          return describeUnimplementedCase("skills");
        case "states":
          return describeUnimplementedCase("states");
        default: {
          const params = [];
          ...
          params.push(
            name,
            `${CGMV.Encyclopedia.DescriptionText}: ${description}`,
          );
          if (data._sketch) {
            params.push(`${CGMV.Encyclopedia.SketchText}: `);
          }
          return params.join(", ");
        }
      }
    ...
    Patcher.patch(CGMV_Window_EncyclopediaDisplay.prototype, "activate", {
      postfix() {
        queueMicrotask(() => {
          if (!this.active) {
            return;
          }
          setTextIfChanged(choiceNode, describeEntry(this._symbol, this._data));
        });
      },
    });

    Patcher.patch(CGMV_Window_EncyclopediaDisplay.prototype, "deactivate", {
      postfix() {
        setTextIfChanged(choiceNode, "");
      },
    });

    CGMV_Window_EncyclopediaList.prototype.describeItem = function (index) {
      const data = this._data[index];
      let text = data._discovered
        ? this.getItemName(this._symbol, data._id)
        : CGMV.Encyclopedia.UnknownEntry;
      if (CGMV.Encyclopedia.NumberEntries) {
        text = `${index + 1}. ${text}`;
      }
      return text;
    };
  ...
  if (hasCGMZEncyclopedia) {
    ...
    const describeEntry = (symbol, data) => {
      if (!data._discovered) {
        return CGMZ.Encyclopedia.UnknownEntryDisplay;
      }
      switch (symbol) {
        case "bestiary":
          return describeUnimplementedCase("bestiary");
        case "items": {
          ...
          const effectTracker = CGMZ_Window_EncyclopediaDisplay.prototype
            .populateEffectTracker(item.effects);
          for (const section of CGMZ.Encyclopedia.ItemDisplayInfo) {
            switch (section) {
              ...
              case "Effect Header":
                if (
                  CGMZ_Window_EncyclopediaDisplay.prototype
                    .hasEffects(effectTracker)
                ) {
        ...
        case "states":
          return describeUnimplementedCase("states");
        case "actors":
          return describeUnimplementedCase("actors");
        default: {
          const params = [];
  ...
  if (hasDrillGaugeFloatingPermanentText) {
    Patcher.patch(Game_System.prototype, "drill_GFPT_create", {
      postfix({ args: [id] }) {
        this._drill_GFPT_dataTank[id]._id = id;
      },
    });

    Patcher.patch(Scene_Map.prototype, "drill_GFPT_updateSpriteDelete", {
      postfix() {
        const length = min(
          $gameTemp._drill_GFPT_windowTank.length,
          drillGaugeFloatingPermanentTextNode.childNodes.length,
        );
        for (let i = 0; i < length; i++) {
          if ($gameTemp._drill_GFPT_windowTank[i] === null) {
            const node = drillGaugeFloatingPermanentTextNode.childNodes[i];
    ...
    Patcher.patch(Drill_GFPT_Window.prototype, "drill_updateBallistics", {
      postfix() {
        const id = this._drill_data._id;
        const node = createChild(drillGaugeFloatingPermanentTextNode, id);
        setTextIfChanged(node, this.contentsOpacity === 0 ? "" : this._text);
      },
    });
  ...
  if (hasPKDSimpleQuestSystem) {
    Window_SQSQuestsList.prototype.describeItem = function (index) {
      const quest = this.questAt(index);
      if (!quest) {
        return "";
      }
      const shortTitle = stripEscapes(quest.titleForList);
      const title = stripEscapes(quest.title.replace(/^<center>/, ""));
      const lines = [stripEscapes(JSON.parse(quest.getActiveDescription()))];
      for (const task of quest.getVisibleTasks()) {
        let text = stripEscapes(task.text);
        if (task.isComplete()) {
          text += ` (${parameters.completedQuestText})`;
      ...
      return `${shortTitle} ${title}: ${lines.join("\n")}`;
    };
  }

  if (hasQJMapProjectileMZ) {
    const pendingLines = [];

    Patcher.patch(Sprite_QJBulletMZ.prototype, "initialize", {
      postfix() {
        const graphics = this.data.data.img;
        if (graphics && graphics[0] === "T") {
          pendingLines.push(graphics[1].text);
        }
      },
    });
    ...
    Patcher.patch(Game_Map.prototype, "update", {
      postfix() {
        if (pendingLines.length !== 0) {
          setTextIfChanged(qjBulletNode, pendingLines.join("\n"));
          pendingLines.length = 0;
        }
      },
    });

    Patcher.patch(Game_Map.prototype, "removeBulletQJ", {
      postfix() {
        if (Object.keys(this._mapBulletsQJ).length === 0) {
          setTextIfChanged(qjBulletNode, "");
        }
      },
    });
  ...
  if (hasTMSimpleWindow) {
    Patcher.findClass(Window_Base, "Window_Simple", (C) => {
      Patcher.patch(C.prototype, "refresh", {
        postfix() {
          const id = this._id;
          const node = createChild(tmSimpleWindowNode, id - 1);
  ...
  if (hasYEPCreditsPage) {
    Patcher.patch(Window_CreditsPage.prototype, "initialize", {
      postfix() {
        this.deactivate();
      },
    });
  ...
  if (typeof KDCore !== "undefined") {
    const describeKDCoreSprite = (sprite) => {
      if (!sprite) {
        return "";
      }
      if (!sprite.describeSelf) {
        return parameters.debugMode
          ? `Unimplemented: ${sprite.constructor.name}.prototype.describeSelf`
          : "";
      }
      return sprite.describeSelf();
    };

    if (KDCore.Sprite_ItemsList) {
      KDCore.Sprite_ItemsList.prototype.describeItem = function (index) {
        return describeKDCoreSprite(this.itemAt(index));
      };
    }

    if (typeof PKD_SST_LazySmartGridSelector !== "undefined") {
      Patcher.patch(PKD_SST_LazySmartGridSelector.prototype, "setupItems", {
        postfix() {
      ...
      Patcher.patch(PKD_SST_LazySmartGridSelector.prototype, "_onHovered", {
        postfix({ args: [item] }) {
          const text = describeKDCoreSprite(item);
          if (parameters.debugMode) {
            console.debug(`${this.constructor.name}: select ${text}`);
          }
          setTextIfChanged(choiceNode, text);
  ...
  return { stripEscapes, describeObject, internals, setCustomTextOutput };
})();
